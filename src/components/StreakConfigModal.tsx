...
export const StreakConfigModal: React.FC<StreakConfigModalProps> = ({
  ...
  return (
    ...
    >
      <div 
        className="bg-white w-full max-w-lg rounded-2xl border border-slate-150 shadow-2xl overflow-hidden animate-scale-up space-y-0"
      ...
      >
        ...
        {savedSuccessMsg && (
          <div className="bg-emerald-50 border-b border-emerald-100 p-3 px-5 text-emerald-800 text-xs font-bold flex items-center gap-2 animate-fade-in">
        ...
        <div className="p-6 space-y-6 max-h-[80vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <div className="bg-orange-50 border border-orange-100 p-3.5 rounded-xl text-center">
              <div className="flex items-center justify-center gap-1 text-orange-600 font-extrabold text-[10px] uppercase tracking-wider mb-1">
                <Flame size={14} className="animate-pulse" />
                <span>Streak hiện tại</span>
              </div>
              <span className="text-2xl font-black text-orange-600 font-mono">{streakData.currentStreak}</span>
              <span className="text-[10px] text-orange-400 block font-bold">ngày</span>
            </div>

            <div className="bg-amber-50 border border-amber-100 p-3.5 rounded-xl text-center">
              <div className="flex items-center justify-center gap-1 text-amber-600 font-extrabold text-[10px] uppercase tracking-wider mb-1">
                <Trophy size={14} />
                <span>Kỷ lục Streak</span>
              </div>
              <span className="text-2xl font-black text-amber-700 font-mono">{streakData.longestStreak}</span>
              <span className="text-[10px] text-amber-500 block font-bold">ngày</span>
            </div>

            <div className="col-span-2 sm:col-span-1 bg-sky-50 border border-sky-100 p-3.5 rounded-xl text-center">
              <div className="flex items-center justify-center gap-1 text-sky-700 font-extrabold text-[10px] uppercase tracking-wider mb-1">
                <ShieldAlert size={14} />
                <span>Băng bảo vệ</span>
              </div>
              <span className="text-2xl font-black text-sky-700 font-mono">{streakData.freezeCount || 0}</span>
              <span className="text-[10px] text-sky-500 block font-bold">vé có sẵn</span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between p-3.5 bg-slate-50 border border-slate-200/80 rounded-xl">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <span className={`text-[11px] font-extrabold ${isStudiedToday ? 'text-emerald-600' : 'text-amber-600'}`}>
          ...
          <div className="space-y-2 border-t border-slate-100 pt-4">
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center border border-slate-200 rounded-xl overflow-hidden bg-slate-50 focus-within:ring-2 focus-within:ring-orange-500/20">
                <button
                  type="button"
                  onClick={() => handleQuickAddStreak(-1)}
                  className="p-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 transition cursor-pointer"
                >
                  <Minus size={16} />
                </button>
                <input 
                  ...
                  className="w-20 text-center font-mono font-black text-lg bg-transparent text-slate-900 focus:outline-none"
                />
                <button
                  type="button"
                  onClick={() => handleQuickAddStreak(1)}
              ...
              <div className="flex flex-wrap items-center gap-1.5">
                <button
                  ...
                  className="px-2.5 py-1.5 bg-slate-100 hover:bg-orange-50 text-slate-700 hover:text-orange-600 font-bold text-xs rounded-lg border border-slate-200 transition cursor-pointer"
                >
                  +5 ngày
                </button>
                <button
                ...
                <button
                  ...
                  className="px-2.5 py-1.5 bg-orange-100 hover:bg-orange-200 text-orange-800 font-extrabold text-xs rounded-lg transition cursor-pointer"
                >
                  30 ngày 🔥
                </button>
              </div>
          ...
          <div className="space-y-2 border-t border-slate-100 pt-4">
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center border border-slate-200 rounded-xl overflow-hidden bg-slate-50 focus-within:ring-2 focus-within:ring-sky-500/20">
                <button
                  type="button"
              ...
              <div className="flex flex-wrap items-center gap-1.5">
                <button
                  ...
                  className="px-2.5 py-1.5 bg-sky-50 hover:bg-sky-100 text-sky-700 font-bold text-xs rounded-lg border border-sky-200 transition cursor-pointer"
                >
                  Mặc định (2 vé)
                </button>
                <button
                  ...
                  className="px-2.5 py-1.5 bg-sky-100 hover:bg-sky-200 text-sky-800 font-extrabold text-xs rounded-lg transition cursor-pointer"
                >
                  +5 vé 🧊
                </button>
              </div>
          ...
          <div className="border-t border-slate-100 pt-4 flex items-center justify-between">
            <div>
              <span className="text-xs font-bold text-slate-700 block">Khôi phục chuỗi về 0</span>
              <span className="text-[10px] text-slate-400">Xóa dữ liệu chuỗi Streak hiện tại nếu muốn học lại từ đầu</span>
            </div>
            <button
              type="button"
              onClick={handleResetStreakToZero}
              className="px-3.5 py-2 bg-rose-50 hover:bg-rose-100 text-rose-700 font-extrabold text-xs rounded-xl transition border border-rose-200 flex items-center gap-1.5 cursor-pointer shrink-0"
            >
              <RotateCcw size={14} />
        ...
        <div className="bg-slate-50 p-4 border-t border-slate-150 flex items-center justify-end gap-2">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 text-xs font-bold text-slate-600 bg-white hover:bg-slate-100 border border-slate-200 rounded-xl cursor-pointer transition"
          >
            Hủy bỏ
          </button>
          <button
